...
Аудиопотоки каждой голосовой конференции проходят только через один определённый сервер. Выбор сервера происходит поэтапно:

- Пользователь (далее - автор голосовой конференции) отправляет HTTP-запрос на создание голосовой конференции
- При помощи Geo Based DNS определяется ближайший к автору голосовой конференции сервер 
- HTTP-запрос на создание голосовой конференции отправляется затем уже на определённый сервер
- Сервер получает данный запрос, отправляет запрос в СУБД на создание голосовой конференции, в котором указывает свой IP-адрес (IP-адрес сервера указывается в поле host таблицы conference), и отвечает пользователю сообщением об успешном создании голосовой конференции (или сообщением об ошибке)
...
- DNS-сервер определяет ближайший к пользователю сервер и в ответ на полученный запрос отправляет его IP-адрес
- Пользователь отправляет HTTP-запрос на подключение к голосовой конференции уже на определённый сервер
- Сервер получает данный запрос, отправляет запрос в СУБД на получение информации об искомой голосовой конференции (IP-адреса сервера, через который осуществляется трансляция её потокового аудио; список пользователей, подключённых в данный момент к голосовой конференции; и т.п.), и отправляет полученные данные об искомой голосовой конференции (или сообщение об ошибке) в ответ на запрос пользователя
- Пользователь, зная IP-адрес сервера, через который осуществляется трансляция потокового аудио, устанавливает соединение с ним по протоколу UDP и начинает трансляцию и получение потокового аудио голосовой конференции
