...
Аудиопотоки каждой голосовой конференции проходят только через один определённый сервер (бекенд). Выбор сервера происходит поэтапно:
...
- DNS-сервер определяет ближайший к пользователю сервер (фронтенд) и в ответ на полученный запрос отправляет его IP-адрес
- Пользователь отправляет HTTP-запрос на создание голосовой конференции уже на определённый сервер (фронтенд)
- Фронтенд получает данный запрос, выполняет поиск ближайшего менее нагруженного сервера (бекенда), выполняет переадресацию запроса на него
- Сервер (бекенд) получает данный переадресованный запрос, отправляет запрос в СУБД на создание голосовой конференции, в котором указывает свой IP-адрес (IP-адрес сервера указывается в поле host таблицы conference), и отвечает пользователю сообщением об успешном создании голосовой конференции (или сообщением об ошибке)
